...
namespace Mekiasm
{
    ...
    public class ILibrary
    {
        ...
        //根据两个位置的坐标获取朝向
        public static int getFace(Point3 start, Point3 end)
        {
            if (end.Z == start.Z + 1) return 0;//前
            else if (end.X == start.X + 1) return 1;//右侧
            else if (end.Z == start.Z - 1) return 2;//后
            else if (end.X == start.X - 1) return 3;//左侧
            else if (end.Y == start.Y + 1) return 4;//上
            else if (end.Y == start.Y - 1) return 5;//下
            return -1;//有问题
        }
        //通过Face获取下一个连接点的Point
        public static Point3 getNextPointByFace(Point3 startPoint, int face)
        {
            switch (face)
            {
                case 0: return startPoint + Point3.UnitZ;
                case 1: return startPoint + Point3.UnitX;
                case 2: return startPoint - Point3.UnitZ;
                case 3: return startPoint - Point3.UnitX;
                case 4: return startPoint + Point3.UnitY;
                case 5: return startPoint - Point3.UnitY;
                default: return Point3.Zero;
            }
        }
