...
namespace Game
{
    public class XjJeiManager
    {
        ...
        public void init()
        {
            ...
            if (isNewBlock)
            {
                XElement item = ContentManager.CombineXml(new XElement("NewBlocks"), ModsManager.GetEntries(".xjdb"));
                ...
                foreach (Block block in BlocksManager.Blocks)
                {
                    ...
                    if (XjJeiManager.is_NewSC && LabelWidget.Strings.ContainsKey(ll))
                    {
                        ll = LabelWidget.Strings[ll];
                    }
