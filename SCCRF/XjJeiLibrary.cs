...
namespace Game
{
    public class XjJeiLibrary
    {
        ...
        public static void moveInventoryByCraft(IInventory srcInventory, IInventory toInventory, Dictionary<int, int> items, ComponentGui gui)
        {
            ...
            { //不能全部转移
                gui.DisplaySmallMessage("背包空间不足，不能转移物品", false, false);
            }
            else
            { //能全部转移
                //先移除全部
                ...
                gui.DisplaySmallMessage("转移成功!", false, false);
            }
        }
        ...
        public static void moveInventory(IInventory a, IInventory b,ComponentPlayer player, GameMode mode)
        {
            ...
            if (remo) player.ComponentGui.DisplaySmallMessage("一键转移成功", false, false);
            else { player.ComponentGui.DisplaySmallMessage("没有需要转移的物品呢", false, false); }
        }
    }
}
