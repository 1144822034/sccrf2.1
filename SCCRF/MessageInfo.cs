                    ...
                    if (parsedata.Count > 0) {
                        ...
                        switch (cmd) {
                            ...
                            case "changeweather":
                                ...
                                {
                                    SubsystemWeather subsystemWeather = player.Project.FindSubsystem<SubsystemWeather>();
                                    SubsystemWeather.Shaft shaft = new SubsystemWeather.Shaft();
                                    shaft.Point = new Point2(Terrain.ToCell(player.ComponentBody.Position.X), Terrain.ToCell(player.ComponentBody.Position.Z));
                                    if (parsedata[1] == "rain") shaft.PrecipitationType = PrecipitationType.Rain;
                                    else if (parsedata[1] == "snow") shaft.PrecipitationType = PrecipitationType.Snow;
                                    else if (parsedata[1] == "sun") {
                                        subsystemWeather.FreeShaft(shaft);
                                    }
                                    else { addMessage($"不能识别的天气类型'{parsedata[1]}'"); break; }
                                    shaft.ParticleSystem = new PrecipitationShaftParticleSystem(new Game.Random());
                                    subsystemWeather.AllocateShaft(player.View,shaft.Point); 
                                    addMessage("更改天气成功");
                                }
                                else {
