...
namespace Game
{
    public class CellInfoShow: Component, IUpdateable,IDrawable
    {
        ...
        public override void Load(ValuesDictionary valuesDictionary, IdToEntityMap idToEntityMap)
        {
            ...
            flatBatch.BlendState = BlendState.Opaque;
            flatBatch.DepthStencilState = DepthStencilState.DepthWrite;
            ...
            if (componentPlayer != null)
            {
                ...
                componentPlayer.View.GameWidget.Children.Find<CanvasWidget>("ControlsContainer").Children.Add(infoDialog);
                componentPlayer.View.GameWidget.Children.Find<CanvasWidget>("ControlsContainer").Children.Add(messageInfo);
                componentPlayer.View.GameWidget.Children.Find<CanvasWidget>("ControlsContainer").Children.Add(moreInfo);
                componentPlayer.View.GameWidget.Children.Find<StackPanelWidget>("RightControlsContainer").Children.Add(msginfobtn);
                componentPlayer.View.GameWidget.Children.Find<CanvasWidget>("ControlsContainer").Children.Add(jumpcanvas);
            }
        ...
        public override void Dispose()
        {
            ...
            messageInfo.Dispose();
            moreInfo.Dispose();
            componentPlayer.View.GameWidget.Children.Find<CanvasWidget>("ControlsContainer").Children.Remove(infoDialog);
            componentPlayer.View.GameWidget.Children.Find<CanvasWidget>("ControlsContainer").Children.Remove(messageInfo);
            componentPlayer.View.GameWidget.Children.Find<CanvasWidget>("ControlsContainer").Children.Remove(moreInfo);
            jumpcanvas.Children.Remove(touch);
            jumpcanvas.Children.Remove(jump);
            componentPlayer.View.GameWidget.Children.Find<CanvasWidget>("ControlsContainer").Children.Remove(jumpcanvas);
            componentPlayer.View.GameWidget.Children.Find<StackPanelWidget>("RightControlsContainer").Children.Remove(msginfobtn);            
            infoDialog = null;
            messageInfo = null;
            moreInfo = null;
            msginfobtn = null;
        ...
        public void Draw(Camera camera, int drawOrder)
        {
            ...
            {               
                ...
                modelsRenderer.PrimitivesRenderer.FontBatch(ContentManager.Get<BitmapFont>("Fonts/SignFont"), 1, DepthStencilState.DepthWrite, null, null, SamplerState.PointWrap).QueueText(markFLag.name + $"({f:0.00})米",vector,right,down,markFLag.color, TextAnchor.Top);
        ...
        public void Update(float dt)
        {
            ...
            if (msginfobtn.IsClicked|| componentPlayer.View.Input.IsKeyDownOnce(Key.Tab)) { //消息点击
            ...
            if (componentPlayer.View.Input.IsKeyDownOnce(Key.B)) {
            ...
            if (messageInfo.IsVisible) {
                ...
                {
                    if (componentPlayer.View.Input.IsKeyDownOnce(Key.UpArrow))
                    {
                    ...
                    else if (componentPlayer.View.Input.IsKeyDownOnce(Key.DownArrow))
                    {
            ...
            if (block.Durability > 0)
            {
                ...
                if (XjJeiManager.is_NewSC && LabelWidget.Strings.ContainsKey(disname))
                {
                    disname = LabelWidget.Strings[disname];
                }
                int durty = 0;
                int damage = block.GetDamage(componentPlayer.ComponentMiner.ActiveBlockValue);
                //durty = block.Durability - Terrain.ExtractData(componentPlayer.ComponentMiner.ActiveBlockValue) / 16;
            ...
            terrainray = componentPlayer.HighlightRaycastResult;            
            foreach (KeyValuePair<ComponentCreature, bool> creature in creatureSpawn.m_creatures)
            {
                ...
                else if (creature.Key.DisplayName.Contains("玩家") || creature.Key.DisplayName.Contains("Player")) continue;
                Ray ray = new Ray(componentPlayer.View.ActiveCamera.ViewPosition, componentPlayer.View.ActiveCamera.ViewDirection);
                if ((Raymm = ray.Intersection(creature.Key.ComponentBody.BoundingBox)).HasValue)
                {
